...
DEEPSEEK_API_KEY = os.environ.get('OPENROUTER_API_KEY')  # Используем OPENROUTER_API_KEY как DEEPSEEK_API_KEY
...
if not DEEPSEEK_API_KEY:
    logger.warning("DEEPSEEK_API_KEY (OPENROUTER_API_KEY) не установлен!")
...
def create_menu():
    ...
    markup.add(KeyboardButton('📝 Текстовый запрос'))
    markup.add(KeyboardButton('📷 Фото'), KeyboardButton('📚 История'))
...
def query_deepseek_api(prompt):
    ...
    if not DEEPSEEK_API_KEY:
        ...
        return "❌ Ключ API DeepSeek не установлен. Не удалось получить ответ от ИИ."
...
def handle_ask_question(message):
    try:
        logger.info(f"Обработка 'Текстовый запрос' от {message.chat.id}")
...
def handle_photo(message):
    try:
        ...
        # Получаем ответ от DeepSeek по распознанному тексту
